...
from devito.ir import (Forward, GuardBoundNext, Queue, Vector, SEQUENTIAL,
                       ...
                       ReleaseLock, normalize_syncs, CudaWaitEvent, CudaFireEvent,
                       CudaPrefetchUpdate, CudaFetchUpdate, CudaWithEvent)
from devito.symbolics import uxreplace
from devito.tools import flatten, is_integer, timed_pass
from devito.types import CustomDimension, Lock, CudaEvent

__all__ = ['CudaTasker', 'CudaStreaming']
            ...
            for c1 in clusters:
                ...
                for f in may_require_lock:
                    ...
                    try:
                        ...
                        name = self.sregistry.make_name(prefix='ev')
                        lock = locks[f] = CudaEvent(name=name)
                    ...
                    for w in writes:
                        ...
                        waits[c1].append(CudaWaitEvent(f, lock))
...
class CudaStreaming(Asynchronous):
    ...
    @timed_pass(name='cuda-streaming')
    def process(self, clusters):
        debug("performing CUDA streaming analysis..")
        processed = super().process(clusters)
        return processed
    ...
    def _actions_from_init(self, cluster, prefix, actions):
        ...
        actions[cluster].syncs[pd].append(CudaFetchUpdate(function, None, d, size, target, 0))
    ...
    def _actions_from_update_memcpy(self, cluster, clusters, prefix, actions):
        ...
        syncs = {d: [CudaPrefetchUpdate(function, handle, d, 1, target, tstore)]}
        ...
        actions[first].syncs[d].append(CudaWaitEvent(function, handle))

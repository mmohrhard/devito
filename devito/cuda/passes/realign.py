...
def realign_iet(iet):
    """
    Attempt to realign the final iteration dimensions in an IET to better
    suit the GPU's cachelines.
    ...
    This way, the CUDA threads in each warp are reading and writing a centre
    point that is aligned with the 256-byte aligned grid. Granted, they'll
    usually also be reading misaligned points along the most frequently-changing
    axis in realistic workloads,
    """
    ...
    all_dims = set(flatten([
        expr.expr.lhs.function.dimensions[-1]
        for expr in exs
        if len(expr.expr.lhs.function.dimensions) > 0]))
    ...
    used_index_map = {d: list(set(flatten([
        expr.expr.lhs.indices[d]
        for expr in exprs
        if d in expr.expr.lhs.indices._getters]))) for d in index_map.keys()}

    # for now, give up early if anything has multiple used indices
    if any([len(x) > 1 for x in used_index_map.values()]):
        return iet

    offset_map = {d: (-uxreplace(used_index_map[d][0], {index_map[d]: Number(0)}))
                  for d in all_dims
                  if d in used_index_map and len(used_index_map[d]) > 0}

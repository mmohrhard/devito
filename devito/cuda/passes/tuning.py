...
def kernel_tuning(iet, **kwargs):
    ...
    grouped = {n: list(set([c for c in kernel_calls if c.name == n]))
    ...
    for call in unique:
        ...
        subblock_parameters = ['sub_block' + suffix[i]
                               for i in range(len(call.preferred_sub_block)
                                              if call.preferred_sub_block is not None
                                              else 0)]
        setup_lambda += 'return program.kernel("%s").instantiate(%s); }' % (
            call.name,
            ','.join(block_parameters + subblock_parameters + [
                ccode(x.rhs)

...
from devito.ir.iet.nodes import DummyExpr
...
from devito.sycl.nodes import SyclKernel, SyclKernelLaunch
from devito.sycl.types import (
    SyclSpecializationConstant,
    SyclSpecializationConstantDefinition,
...
from devito.types.basic import IndexedData, Symbol
...
def sycl_linearize(graph, **kwargs):
    """
    ...
    sycl_linearization(graph, track=track, **kwargs)
...
@iet_pass
def sycl_linearization(iet, **kwargs):
    """
    ...
    track = kwargs["track"]
    spec_constants = kwargs.get("use-specialization-constants", False)
    ...
    debug("performing sycl linearization")
    iet, headers, _globals = linearize_accesses(
        iet, key, track, sregistry, spec_constants
    )
...
def linearize_accesses(iet, key, track, sregistry: SymbolRegistry, spec_constants: bool):
    """
    ...
    kernels = FindNodes(SyclKernel).visit(iet)
    ...
    for f, v in imapper.items():
        for d in v:
            ...
            try:
                ...
                if spec_constants:
                    _globals.append(
                        SyclSpecializationConstantDefinition(name=name, dtype=np.int64)
                    )
    ...
    for f in selected:
        if track[f].cbk is None:
            header, track[f].cbk = _generate_macro(
                f, mapper[f], sregistry, spec_constants
            )
    ...
    kmapper = {k: Uxreplace(mapper).visit(k) for k in kernels}
    iet = Transformer(kmapper).visit(iet)
    ...
    for f, v in track.items():
        ...
        if f in candidates or release:
            if f in defines and not isinstance(iet, SyclKernel):
    ...
    if stmts0:
        if spec_constants:
            writes = [
                SyclSpecializationConstantWrite(
                    SyclSpecializationConstant(name=s.expr.lhs.name), s.expr.rhs
                )
                for s in stmts1
            ]
            mapper = {
                k: k._rebuild(constant_writes=writes)
                for k in FindNodes(SyclKernelLaunch).visit(iet)
            }
            iet = Transformer(mapper).visit(iet)
            # we need to add the specialisation constant writes
            # to every kernel launch that requires them
            stmts0 = filter_ordered(stmts0) + [BlankLine]
            body = iet.body._rebuild(body=tuple(stmts0) + iet.body.body)
...
def _(f, d, sregistry):
    ...
    return (
        CudaConstantDecl(s, np.int64),
        CudaConstantWrite(s, expr),
        DummyExpr(s, expr, init=True),
    )
...
@singledispatch
def _generate_macro(f, szs, sregistry, spec_constants):
    return


@_generate_macro.register(DiscreteFunction)
@_generate_macro.register(Array)
def _(f, szs, sregistry, spec_constants):
    ...
    expr = sum(
        [
            MacroArgument("_" + d0.name)
            * (SyclSpecializationConstant(szs[d1].name) if spec_constants else szs[d1])

...
def realign_iet(iet):
    """
    Attempt to realign the final iteration dimensions in an IET to
    better suit the GPU's cachelines.
            ...
            set(
                flatten(
                    [
                        ...
                        for expr in exprs
                        if d in expr.expr.lhs.indices._getters
                    ]
                )
            )
        )
        for d in index_map.keys()
    }
    ...
    offset_map = {
        d: (-uxreplace(used_index_map[d][0], {index_map[d]: Number(0)}))
        for d in all_dims
...
class IterationLimitTranslator(Visitor):
    ...
    def visit_Iteration(self, o, **kwargs):
        if o.dim in self._dim_mapper:
            return o._rebuild(
                limits=(
                    o.limits[0] - self._dim_mapper[o.dim],

...
from devito.sycl.nodes import SyclAlloc, SyclDealloc
...
from devito.ir.iet.visitors import FindSymbols, MapExprStmts, Transformer
...
__all__ = ["DeviceSyclDataManager"]


class DeviceSyclDataManager(DataManager):
    lang = SyclBB
    ...
    def _alloc_local_array_on_high_bw_mem(self, site, obj, storage, devicerm=None):
        ...
        allocs = SyclAlloc(obj, nbytes, queue=SyclQueue())
        free = SyclDealloc(obj, queue=SyclQueue())
    ...
    def _alloc_mapped_array_on_high_bw_mem(self, site, obj, storage, *args):
        ...
        alloc = SyclAlloc(obj, nbytes_arg, queue=SyclQueue())
        free = SyclDealloc(obj, queue=SyclQueue())
    ...
    def place_casts(self, iet, **kwargs):
        ...
        casts = [
            self.lang.PointerCast(i.function, obj=i) for i in bases if i not in defines
        ]

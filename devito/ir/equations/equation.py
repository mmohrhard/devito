...
class Operation(Tag):
    ...
    @classmethod
    def detect(cls, expr):
        reduction_mapper = {Inc: OpInc, ReduceMax: OpMax, ReduceMin: OpMin}
        try:
...
class LoweredEq(IREq):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if len(args) == 1 and isinstance(args[0], LoweredEq):
            ...
            cluster_fusion_key = expr.cluster_fusion_key
        elif len(args) == 2:
            ...
            raise ValueError(
                "Cannot construct LoweredEq from args=%s "
        ...
        intervals = IntervalGroup(
            [Interval(d, 0, 0) for d in iterators], relations=ordering.relations
        )
        ...
        expr._cluster_fusion_key = cluster_fusion_key
    ...
    def xreplace(self, rules):
        return LoweredEq(
            self.lhs.xreplace(rules), self.rhs.xreplace(rules), **self.state
        )
    ...
    def __new__(cls, *args, **kwargs):
        if len(args) == 1:
            ...
            if isinstance(input_expr, IREq):
                ...
                expr._cluster_fusion_key = kwargs.get("cluster_fusion_key", None)
                try:
                    expr._suppress_fusion_dims = input_expr.suppress_fusion_dims
                except:
                    pass
                try:
                    expr._cluster_fusion_key = input_expr.cluster_fusion_key
                except:
                    pass
        elif len(args) == 2:
            ...
            raise ValueError(
                "Cannot construct ClusterizedEq from args=%s "
                "and kwargs=%s" % (str(args), str(kwargs))
            )
        return expr
